...
- [x] 6.1 Add a project-aware `storage_volume_import_iso` tool that validates a bounded Base64 ISO artifact and streams it through the official custom-ISO volume import operation.
...
>
> 2026-08-09 real-MicroOS acceptance: downloaded the current openSUSE Tumbleweed ContainerHost qcow2 stable alias (`x-media-version: 20260806`), verified its upstream SHA-256 and qcow2 format, wrapped it as the reference Incus VM image (`metadata.yaml` + unchanged `rootfs.img`), and imported it through the deployed MCP bridge. Generated an ephemeral-key Combustion script from the local `proxmox_homelab` reference, then authored a labelled `combustion` ISO containing `/combustion/script` with the reference-exact `xorriso -as mkisofs -V combustion -graft-points` command. The Base64 artifact import created an `iso` custom volume and the run-unique VM booted latest MicroOS with DHCP and readable serial console. Both USB and standard custom-ISO CD-ROM attachment forms reached normal MicroOS serial login, but neither emitted the Combustion completion marker and the ephemeral SSH key was rejected; snapshot/restart acceptance was therefore not attempted. Every test VM, ISO volume, and image was explicitly removed and independently confirmed absent after each attempt. No pool/network/appliance/certificate/non-disposable resource was changed. Keep 8.3 open until the latest-image Combustion-media discovery gap is resolved and successful guest bootstrap, snapshot, restart, and cleanup are observed.
