...
- **NOTE**: Spike (task 1.1) confirmed the appliance API is proxied through the Incus application at `/os/1.0/...` (the fallback listener trims the `/os` prefix and requires a trusted client certificate); the earlier `/1.0/os/*` probe failed because the prefix is `/os/1.0/*`, not `/1.0/os/*`. Reachable surface for system tools: `GET/POST /os/1.0/system/update` and `POST /os/1.0/system/update/:check` (OS update status/trigger), `GET /os/1.0/applications` + per-application lifecycle actions, `GET/PUT /os/1.0/system/security` (recovery keys, TPM status). Debug endpoints (`/1.0/debug/*`) and internal endpoints (`/internal/*`) are explicitly unstable and out of the tool surface.
